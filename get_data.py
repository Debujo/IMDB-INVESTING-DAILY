...
def get_stocks_data():
    ...
    for x in stocks:
        try:
            ...
            mean2_df = stock[floor(n_days-(config.moving_average/3)):n_days] # Filters the Data Frame to get the last mean_number/2 days
            mean1_simple = mean1_df.Close.mean() # Long term mean
            mean2_simple = mean2_df.Close.mean() # Short term mean

            if mean1_simple > mean2_simple & (stock.Close[n_days]/stock.Open[n_days-1]) > config.turn_up_percentage & (stock.Open[n_days-1]-stock.Close[n_days-1]) >= 0:
                #Send Alert notification()
                print("TURNING STOCK: " + x +"!")
                notification.send_email(x)
            csv_save(x)
        except:
            pass
